...
class RanksGenerator(GenericDataPreprocessor):
    def __init__(self):
        self.ranks = np.load(os.path.join(
            'data/', 'ranks_mapping.npy'))
    ...
    def transform(self, X):
        ...
        for i, c in enumerate(var_cols):
            X[c] = X[c].map(self.ranks[i]).astype(np.uint32)
...
data_loader.preprocess(DropColumns, IsUnique, RanksGenerator, ToNumpy)
